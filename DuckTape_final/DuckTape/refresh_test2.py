...
def restart():
    ...
    button_refresh_text = button_font.render("Refresh", True, TEXT_COLOR)
    button_refresh_rect = pygame.Rect(300, 550, 200, 50)  # Position the button
    # Simulated list of dates
    dates = list(images.keys())
    date_buttons = [pygame.Rect(300, 150 + i * 60, 250, 40) for i in range(len(dates))]
    ...
    def draw_home_page():
        ...
        if button_refresh_rect.collidepoint(mouse_pos):
            draw_button(button_refresh_rect, button_refresh_text, BUTTON_HOVER_COLOR)
        else:
            draw_button(button_refresh_rect, button_refresh_text, BUTTON_COLOR)

        
    def refresh_data():
        global images, responses, dates  # Use global variables to update the history and latest images
        images = {}
        responses = {}
        dates = list(db.history())  # Get the latest dates
        for date in dates:
            record = db.record(date)
            if record:
                images[date] = record[0][0]  # Assuming the first column is the image path
    ...
    def draw_latest_image_section():
        ...
        if latest_record:
            ...
            Response_text = title_font.render("Response", True, HEADER_COLOR)
            screen.blit(Response_text, Response_text.get_rect(topright=(width - 10, 100)))
            Date_text = title_font.render(f"{dates[-1]}", True, HEADER_COLOR)
            screen.blit(Date_text, Date_text.get_rect(midtop=(width // 2, 150)))
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                elif button_refresh_rect.collidepoint(event.pos):
